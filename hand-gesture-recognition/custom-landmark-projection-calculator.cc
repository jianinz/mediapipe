...
// Projects normalized landmarks in a rectangle to its a norm rect's coordinates. The
...
// Usage example:
// node {
//   calculator: "CustomLandmarkProjectionCalculator"
...
// }
class CustomLandmarkProjectionCalculator : public CalculatorBase {
 public:
  ...
  ::mediapipe::Status Process(CalculatorContext* cc) override {
    ...
    for (; input_id != cc->Inputs().EndId(kLandmarksTag);
      ...
      for (int i = 0; i < input_landmarks.landmark_size(); ++i) {
        ...
        const float x = landmark.x() - input_rect.x_center();
        const float y = landmark.y() - input_rect.y_center();
...
REGISTER_CALCULATOR(CustomLandmarkProjectionCalculator);

...
public class PacketCreator {
   ...
   */
  public Packet createYuvImageFrame(ByteBuffer yBuffer, ByteBuffer uBuffer, ByteBuffer vBuffer, int yStride, int uStride, int vStride, int width, int height) {
    if (yBuffer.capacity() != width * height) {
      throw new RuntimeException("buffer doesn't have the correct size.");
    }
    return Packet.create(
        nativeCreateYuvImageFrame(mediapipeGraph.getNativeHandle(), yBuffer, uBuffer, vBuffer, yStride, uStride, vStride, width, height));
  }
  ...
  private native long nativeCreateYuvImageFrame(
      long context, ByteBuffer yBuffer, ByteBuffer uBuffer, ByteBuffer vBuffer, int yStride, int uStride, int vStride, int width, int height);
